...
inline void make_input_features(const Position& position, features1_t* features1, features2_t* features2) {
	DType(*features2_hand)[ColorNum][MAX_PIECES_IN_HAND_SUM][1] = reinterpret_cast<DType(*)[ColorNum][MAX_PIECES_IN_HAND_SUM][1]>(features2);
	...
	for (Color c = Black; c < ColorNum; ++c) {
		...
		for (HandPiece hp = HPawn; hp < HandPieceNum; ++hp) {
			...
			std::fill_n((*features2_hand)[c2][p], 1 * num, 1);
	...
	// is check
	if (position.inCheck()) {
		std::fill_n((*features2)[MAX_FEATURES2_HAND_NUM], 1, 1);
	}
}
