...
def main(*argv, optuna_trial=None):
    ...
    if args.swa_scheduler:
        ...
        swa_scheduler = eval('' + args.swa_scheduler.replace('(', '(optimizer,'))
    if args.use_swa:
    ...
    for e in range(args.epoch):
        ...
        if args.swa_scheduler:
            logging.info('swa_scheduler lr={}'.format(swa_scheduler.get_last_lr()))
        epoch += 1
        steps_epoch = 0
        sum_loss1_epoch = 0
        sum_loss2_epoch = 0
        ...
        logging.debug(f"in the end of epoch: {epoch}; e: {e}")
        if args.use_swa and epoch >= args.swa_start_epoch:
