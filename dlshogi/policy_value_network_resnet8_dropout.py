...
from .resnet import BasicBlock, BasicBlockDropout, ResNet
...
class PolicyValueNetwork(ResNet):
    def __init__(self, use_aux=False):
        ...
        self.inplanes = 64
        self.l2 = self._make_layer(BasicBlockDropout,  64, 2, activation=nn.LeakyReLU(inplace=True))
        self.l3 = self._make_layer(BasicBlockDropout, 128, 2, activation=nn.LeakyReLU(inplace=True))
        self.l4 = self._make_layer(BasicBlockDropout, 256, 2, activation=nn.LeakyReLU(inplace=True))
        self.l5 = self._make_layer(BasicBlockDropout, 512, 2, activation=nn.LeakyReLU(inplace=True))
        # policy network
        k = 512
