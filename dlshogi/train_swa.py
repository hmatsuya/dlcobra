...
def main(*argv, optuna_trial=None):
    ...
    parser.add_argument('--swa_scheduler', help='SWA learning rate scheduler')
    ...
    parser.add_argument('--swa_step', type=int, default=20) # should be same as the T_max*2 of swa_scheduler
    ...
    parser.add_argument('--update_batchnorm', action='store_true', help='update batchnorm')
    ...
    if args.swa_scheduler:
        logging.info('swa_scheduler {}'.format(args.swa_scheduler))
    if args.use_critic:
    ...
    if args.lr_scheduler:
        if args.lr_scheduler[-1] != ')':
            args.lr_scheduler += '()'
        scheduler = eval('optim.lr_scheduler.' + args.lr_scheduler.replace('(', '(optimizer,', 1))
        # scheduler = eval('' + args.lr_scheduler.replace('(', '(optimizer,'))
    if args.swa_scheduler:
        if args.swa_scheduler[-1] != ')':
            args.swa_scheduler += '()'
        # swa_scheduler = eval('optim.lr_scheduler.' + args.lr_scheduler.replace('(', '(optimizer,'))
        swa_scheduler = eval('' + args.swa_scheduler.replace('(', '(optimizer,', 1))
    if args.use_swa:
    ...
    if args.resume:
        ...
        # t = checkpoint['t']
        t = 0
        swa_epoch = checkpoint['swa_epoch'] if 'swa_epoch' in checkpoint else 0
        if 'model' in checkpoint:
            ...
            if args.swa_scheduler and not args.reset_scheduler and 'swa_scheduler' in checkpoint:
    ...
    def save_checkpoint():
        ...
        if args.swa_scheduler:
            ...
            checkpoint['swa_epoch'] = swa_epoch
    ...
    def save_model(onnx=False):
        ...
        # if args.use_swa and epoch >= args.swa_start_epoch:
        if args.update_batchnorm and args.use_swa and epoch >= args.swa_start_epoch:
            update_batchnorm()
            # logging.info('Updating batch normalization')
            # forward_ = swa_model.forward
            # swa_model.forward = lambda x : forward_(**x)
            ...
            #     test_accuracy1, test_accuracy2,
            #     test_entropy1, test_entropy2))
            logging.info('Saving the SWA model to {}'.format(model_path + ".swa"))
            serializers.save_npz(model_path + ".swa", swa_model.module)
    ...
    for e in range(args.epoch):
        ...
        if args.swa_scheduler:
            logging.info('swa_scheduler lr={}'.format(swa_scheduler.get_last_lr()[0]))
        epoch += 1
        steps_epoch = 0
        sum_loss1_epoch = 0
        sum_loss2_epoch = 0
        ...
        for x1, x2, t1, t2, value in train_dataloader:
            if steps_epoch >= (train_len // args.batchsize):
                break
            t += 1
            steps += 1
            with torch.cuda.amp.autocast(enabled=args.use_amp):
            ...
            if (t % eval_interval == 0) or (t == 1):
                ...
                with torch.no_grad():
                    ...
                    if loss.item() >= min_loss:
                        ...
                        no_progress = 0
                        # min_loss = loss.item() # Use only SWA loss
                        # if optuna_step >= 30:
                    ...
                    if t > 1:
                        ...
                        if args.use_swa and epoch >= args.swa_start_epoch:
                            logging.info(f"swa_epoch: {swa_epoch}")
                            # if (swa_epoch + args.swa_step / 2) % args.swa_step == 0:
                            if swa_epoch % 10 == 9:
                                logging.info("Updating SWA model parameters.")
        ...
        if bn_loss:
            min_loss = bn_loss
        logging.debug(f'min_loss: {min_loss}')
