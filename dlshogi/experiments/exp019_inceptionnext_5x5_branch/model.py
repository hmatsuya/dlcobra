"""Isotropic InceptionNeXt architecture with 5x5 depthwise branch for Shogi.

depths=[10], dims=[190]

Based on exp015 but with 5 parallel depthwise branches instead of 4:
- identity (no conv)
- 3x3 depthwise (local patterns)
- 1x9 depthwise (horizontal, rook/lance)
- 9x1 depthwise (vertical, rook/lance)
- 5x5 depthwise (knight jumps, bishop diagonals) <- NEW

The 5x5 branch improves receptive field for:
- Knight L-shaped jumps (桂馬)
- Bishop diagonal moves (角)
- Extended tactical sequences

Requires dim divisible by 5 (190 instead of 192).
"""
import torch
...
class InceptionNeXtBlock(nn.Module):
    """InceptionNeXt block with 5 parallel depthwise branches + MLP.

    Channel split: dim // 5 each for identity, 3x3, 1x9, 9x1, 5x5.
    Requires dim divisible by 5.
    """

    def __init__(self, dim, expansion=4):
        super().__init__()
        assert dim % 5 == 0, f"dim must be divisible by 5, got {dim}"
        branch_dim = dim // 5
        ...
        self.dw5x5 = nn.Conv2d(branch_dim, branch_dim, kernel_size=5, padding=2, groups=branch_dim)
    ...
    def forward(self, x):
        ...
        x0, x1, x2, x3, x4 = x[:, :b], x[:, b:2*b], x[:, 2*b:3*b], x[:, 3*b:4*b], x[:, 4*b:]
        x1 = self.dw3x3(x1)
        x2 = self.dw1x9(x2)
        x3 = self.dw9x1(x3)
        x4 = self.dw5x5(x4)
