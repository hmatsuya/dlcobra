"""Isotropic InceptionNeXt with reduced MLP expansion (expansion=2).

depths=[20], dims=[192], expansion=2

Same as exp012 but MLP expansion reduced from 4→2 (192→384→192 instead of 192→768→192).
Targets the dominant bottleneck identified in profiling: addmm was 59% of CUDA time.
Expected to be ~30% faster than exp012 with slightly fewer parameters.
"""
import torch
...
class PolicyValueNetwork(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(*[InceptionNeXtBlock(dim, expansion=2) for _ in range(depths[0])])
