"""Isotropic InceptionNeXt with lighter value head.

Based on exp015 (depths=[10], dims=[192]) but with value head bottleneck fix:
- Value head uses 2 channels instead of MAX_MOVE_LABEL_NUM (27)
- Reduces parameters by ~500k (~14% of exp015 model)
- Expected: neutral or slight accuracy gain, faster linear layer operations
"""
import torch
...
class PolicyValueNetwork(nn.Module):
    def __init__(
        ...
        dims=(192,),
        fcl=256,
        value_channels=2,  # NEW: reduced from MAX_MOVE_LABEL_NUM (27)
    ):
        super().__init__()
        dim = dims[0]
        ...
        # VALUE HEAD BOTTLENECK FIX: 2 channels instead of 27
        self.value_conv = nn.Conv2d(dim, value_channels, kernel_size=1, bias=False)
        self.value_norm = nn.LayerNorm(value_channels)
        self.value_fc1 = nn.Linear(9 * 9 * value_channels, fcl)  # 162 -> fcl instead of 2187 -> fcl
