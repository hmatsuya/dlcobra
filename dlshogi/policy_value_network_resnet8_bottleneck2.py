...
class PolicyValueNetwork(ResNet):
    def __init__(self, use_aux=False):
        ...
        self.l4 = self._make_layer(Bottleneck, 256, 2, activation=nn.LeakyReLU(inplace=True))
        self.l5 = self._make_layer(Bottleneck, 512, 2, activation=nn.LeakyReLU(inplace=True))
        # policy network
        k = self.inplanes
