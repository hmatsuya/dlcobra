...
class PolicyValueNetwork(ResNet):
    def __init__(self, use_aux=False):
        ...
        self.inplanes = 64
        self.l2 = self._make_layer(BasicBlock,  64, 4, activation=nn.LeakyReLU(inplace=True))
        self.l3 = self._make_layer(BasicBlock, 128, 4, activation=nn.LeakyReLU(inplace=True))
        self.l4 = self._make_layer(BasicBlock, 256, 4, activation=nn.LeakyReLU(inplace=True))
        self.l5 = self._make_layer(BasicBlock, 512, 4, activation=nn.LeakyReLU(inplace=True))
        # policy network
        k = 512
